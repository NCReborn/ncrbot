        ...
        group.setName('bulk')
            ...
            .addSubcommand(subcmd =>
                subcmd.setName('bulkgrant30')
                    .setDescription('Bulk set all current Snapsmiths to 30 unique reactions for milestone testing')
            )
    )
    // ANNOUNCE GROUP
    .addSubcommandGroup(group =>
...
async function execute(interaction) {
    try {
        ...
        // BULK GROUP
            ...
            // BULKGRANT30: Patch to grant 30 unique reactions to all current Snapsmiths
                ...
                await interaction.editReply({
                    content: `Granted 30 reactions to all current Snapsmiths: ${granted.join(', ')}. Run /snapsmithadmin config syncmilestonedays to sync milestones and expiry.`
                });
            }
        }
        // ANNOUNCE GROUP
